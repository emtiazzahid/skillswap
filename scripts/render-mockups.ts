...
const files = (await readdir(mockDir)).filter(
	(f) => f.endsWith('.html') && !f.startsWith('_') && (!only || f.includes(only))
);
...
for (const file of files) {
	...
	for (const [label, vp] of Object.entries(viewports)) {
		...
		console.log(
			`${name} ${label}: ${overflow.length ? 'OVERFLOW: ' + overflow.join(', ') : 'ok'}${errors.length ? ' console errors: ' + errors.join(' | ') : ''}`
		);
		await page.close();
	}
}
